...
function onImportFileSelect(event: Event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (!parsed.dependencies || !parsed.summary) {
        importError.value = 'Invalid scan result: missing "dependencies" or "summary" fields'
        importFileData.value = null
        return
      }
...
async function submitImport() {
  if (!importFileData.value || !project.value) return
  importing.value = true
  try {
    await api.importScan(project.value.id, importFileData.value)
    toast.add({
      severity: 'success',
      summary: 'Scan Imported',
      detail: `Imported scan from ${importFileName.value}`,
      life: 3000
    })
    ...
    toast.add({
      ...
      detail: e.message || 'Failed to import scan result',
      life: 5000
    })
  } finally {
...
<template>
  <div class="project-detail">
    ...
    <template v-else-if="project">
      ...
      <div class="section scan-config-section">
        ...
        <div class="scan-actions">
          ...
          >
            <i class="pi pi-upload"></i> Import CI/CD Scan
          </button>
        </div>
      </div>
    ...
    <Dialog
      ...
      header="Import CI/CD Scan Result"
      :modal="true"
      :style="{ width: '550px' }"
    >
      <div class="import-dialog-content">
        <p class="import-description">
          Upload a <code>report.json</code> file generated by the OrtoPed CLI to import scan results for curation.
        </p>
        ...
        <div class="import-file-area">
          ...
          />
          <button class="btn btn-outline import-file-btn" @click="($refs.importFileInput as HTMLInputElement)?.click()">
        ...
        <div v-if="importFileData" class="import-preview">
          <h4>Scan Preview</h4>
          <div class="preview-grid">
            <div class="preview-item">
              <label>Dependencies</label>
              <span>{{ importFileData.dependencies?.length || 0 }}</span>
            </div>
            <div class="preview-item">
              <label>Resolved</label>
              <span>{{ importFileData.summary?.resolved || 0 }}</span>
            </div>
            <div class="preview-item">
              <label>Unresolved</label>
              <span>{{ importFileData.summary?.unresolved || 0 }}</span>
            </div>
            <div class="preview-item">
              <label>AI Enhanced</label>
              <span>{{ importFileData.aiEnhanced ? 'Yes' : 'No' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        ...
        <button
          class="btn btn-primary"
          @click="submitImport"
          :disabled="!importFileData || importing"
        >
          <i class="pi pi-upload" :class="{ 'pi-spin pi-spinner': importing }"></i>
