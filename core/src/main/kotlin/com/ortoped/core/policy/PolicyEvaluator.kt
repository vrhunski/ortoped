...
class PolicyEvaluator(
    ...
    fun evaluate(scanResult: ScanResult): PolicyReport {
        ...
        for (dependency in scanResult.dependencies) {
            ...
            for (rule in config.rules.filter { it.enabled }) {
                val (matched, violation) = evaluateRule(rule, dependency, effectiveLicense, category)
                
                if (matched) {
                    if (rule.action == RuleAction.ALLOW) {
                        // Explicitly allowed, stop evaluating further rules for this dependency
                        break
                    }
     ...
     * Priority: concluded > AI high confidence > declared > detected > NOASSERTION
     */
    private fun getEffectiveLicense(dependency: Dependency): String {
        ...
        // Declared license
        val declared = dependency.declaredLicenses.firstOrNull()
        if (declared != null) {
            return declared
        }

        // Detected license (from source scanning)
        val detected = dependency.detectedLicenses.firstOrNull()
        if (detected != null) {
            return detected
        }
     ...
     * Returns a pair of (matched, violation?)
     */
    ...
    private fun formatMessage(template: String, dependency: Dependency, license: String): String {
        return template
            ...
            .replace("{{dependency}}", dependency.name)
